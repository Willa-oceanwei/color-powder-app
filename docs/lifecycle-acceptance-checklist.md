# Lifecycle、沖銷與 Tombstone 正式環境驗收清單（Schema v13）

本清單用於 Schema v13 上線驗收。目標是確認 Turso 永遠保留可稽核的業務歷史，且
...
- [ ] 開啟測試 app，登入後確認 database health check 顯示 Schema v13。
...
- [ ] dry-run `客戶名單`，確認每列都有唯一 `客戶編號` 與非空白簡稱。
- [ ] duplicate、validation error 與 conflict 都為 0 後執行 `IMPORT 客戶名單`。
- [ ] IMPORT 後重跑 dry-run，確認全部 unchanged。
- [ ] 從網站新增、改名、停用與恢復測試客戶，確認 Sheet baseline/conflict 保護正常。

## 1.3 Pantone 色號表初始化

- [ ] dry-run `Pantone色號表`，確認每列都有 Pantone 色號與唯一配方編號。
- [ ] duplicate、validation error 與 conflict 都為 0 後執行 `IMPORT Pantone色號表`。
- [ ] IMPORT 後重跑 dry-run，確認全部 unchanged，再從網站新增一筆並驗證 outbox PUSH。

## 1.4 樣品記錄初始化

- [ ] dry-run `樣品記錄`，確認每列都有唯一樣品編號，無 duplicate/conflict 後 initial IMPORT。
- [ ] IMPORT 後重跑 dry-run，確認全部 unchanged。
- [ ] 從網站新增、修改與停用樣品，確認 Sheet outbox 與 tombstone 保護正常。
