...
def generate_production_order_print(order, recipe_row, additional_recipe_rows=None, show_additional_ids=True):
    ...
    number_col_width = 12
    column_offsets = [-1, 2, 2, 3]  # <-- 調整這裡數值即可左右移動欄位
    ...
    info_line = f"編號：{recipe_id}  顏色：{color}   比例：{ratio} g/kg   Pantone：{pantone}"
    ...
    for pid, wgt in zip(colorant_ids, colorant_weights):
        if pid and wgt > 0:
            ...
            for offset, m in zip(column_offsets, multipliers):
                if m > 0:
                    val = wgt * m
                    if val:
    ...
    for offset, m in zip(column_offsets, multipliers):
        if m > 0:
            ...
            if val:
                val_str = str(int(val)) if val.is_integer() else f"{val:.3f}".rstrip('0').rstrip('.')
                total_line += " " * offset + val_str.rjust(number_col_width)
    ...
    if additional_recipe_rows and isinstance(additional_recipe_rows, list):
        for idx, sub in enumerate(additional_recipe_rows, 1):
            ...
            add_ids = [str(sub.get(f"色粉編號{i}", "") or "").strip() for i in range(1, 9)]
            add_weights = [float(sub.get(f"色粉重量{i}", 0) or 0) for i in range(1, 9)]
            ...
            for offset, m in zip(column_offsets, multipliers):
                if m > 0:
                    ...
                    if val:
                        val_str = str(int(val)) if val.is_integer() else f"{val:.3f}".rstrip('0').rstrip('.')
                        sub_total_line += " " * offset + val_str.rjust(number_col_width)
...
# ======== 客戶名單 =========
    ...
    if len(values) > 1:
        df_sheet = pd.DataFrame(values[1:], columns=values[0])
    else:
        df_sheet = pd.DataFrame(columns=columns)
    ...
    # === 下面就可以繼續寫 UI（搜尋、新增、修改...） ===
    st.subheader("➕ 新增 / 修改配方")
    with st.form("recipe_form"):
        col1, col2, col3 = st.columns(3)
        with col1:
            fr["配方編號"] = st.text_input("配方編號", value=fr.get("配方編號", ""))
        with col2:
            fr["顏色"] = st.text_input("顏色", value=fr.get("顏色", ""))
        with col3:
            options = [""] + customer_options
            selected = st.selectbox("客戶", options, index=options.index(f"{fr.get('客戶編號', '')} - {fr.get('客戶名稱', '')}") if f"{fr.get('客戶編號', '')} - {fr.get('客戶名稱', '')}" in options else 0)
            if " - " in selected:
    ...
    if page == "新增生產單":
        ...
        if recipe_id:
            ...
            if additional_recipes.empty:
                ...
                st.markdown(f"##### 附加配方清單（共 {len(additional_recipes)} 筆）")
