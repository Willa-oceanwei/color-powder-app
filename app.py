...
# ============================================================
    ...
    with tab2:
        ...
        # 初始化搜尋關鍵字
        for key in ["recipe_kw_tab2", "customer_kw_tab2", "pantone_kw_tab2"]:
            if key not in st.session_state:
                st.session_state[key] = ""
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            search_recipe_bottom = st.text_input("配方編號", key="recipe_kw_tab2")
        with col2:
            search_customer_bottom = st.text_input("客戶名稱或編號", key="customer_kw_tab2")
        with col3:
            search_pantone_bottom = st.text_input("Pantone色號", key="pantone_kw_tab2")

        recipe_kw = st.session_state.get("recipe_kw_tab2", "").strip()
        ...
        if not page_data.empty:
            ...
            for idx, row in page_data.iterrows():
                with st.expander(f"🔖 {row['配方編號']} - {row['客戶名稱']} - {row['產品名稱'] if '產品名稱' in row else row['顏色']}"):
                    col1, col2 = st.columns(2)
                    ...
                    powder_info = []
                    for i in range(1, 9):
                        powder_id = row.get(f'色粉編號{i}', '')
                        amount = row.get(f'色粉重量{i}', '')
                        if powder_id and str(powder_id).strip():
                            powder_info.append(f"- {powder_id}: {amount}g")
...
# ============================================================
    ...
    with tab3:
            ...
            if selected_code:
               ...
               # ------------------- 確認刪除 -------------------
                ...
                if st.session_state.get("show_edit_recipe_panel") and st.session_state.get("editing_recipe_index") is not None:
                    ...
                    with col10:
                        fr["淨重"] = st.text_input("色粉淨重", value=fr.get("淨重", ""), key="edit_recipe_淨重_tab3")
                    with col11:
                        options = ["g", "kg"]
                        current = fr.get("淨重單位", options[0])
                        if current not in options:
                            current = options[0]
                        fr["淨重單位"] = st.selectbox("單位", options, index=options.index(current), key="edit_recipe_淨重單位_tab3")
                    ...
                    if "edit_num_powder_rows" not in st.session_state:
                        st.session_state.edit_num_powder_rows = max(5, sum(1 for i in range(1, 9) if fr.get(f"色粉編號{i}")))
                    ...
                    with col_btn_add:
                        if st.button("➕ 新增色粉列", key="add_powder_row_edit_tab3"):
                            if st.session_state.edit_num_powder_rows < 8:
                                st.session_state.edit_num_powder_rows += 1
                                st.rerun()
                    with col_btn_remove:
                        if st.button("➖ 減少色粉列", key="remove_powder_row_edit_tab3"):
                            if st.session_state.edit_num_powder_rows > 1:
                                st.session_state.edit_num_powder_rows -= 1
                                st.rerun()
                        ...
                        if st.button("💾 儲存修改", key="save_edit_recipe_btn_tab3"):
                            ...
                            try:
                                ...
                                if not header:
                                    ...
                                    recipe_id = str(df.at[idx, "配方編號"]) if "配方編號" in df.columns else ""
                                    row_num = idx + 2
                                    ...
                                    values_row = [
                                        str(df.at[idx, col]) if (col in df.columns and pd.notna(df.at[idx, col])) else ""
                                        for col in header
                                    ]
                                ...
                                try:
                                    header_len = len(header) if 'header' in locals() else len(df.columns)
        ...
        # 頁面最下方手動載入按鈕
        st.markdown("---")
        if st.button("📥 重新載入配方資料", key="reload_recipe_tab3"):
            st.session_state.df_recipe = load_recipe_data()
...
# ========== Tab 3：Pantone色號表 ==========
        ...
        if search_code:
            ...
            if df_result_pantone.empty and df_result_recipe.empty:
                ...
                if not df_result_pantone.empty:
                    st.markdown(
                        '<div style="font-size:18px; font-family:Arial; color:#f0efa2; line-height:1.2; margin:2px 0;">📋 Pantone 對照表</div>',
                        unsafe_allow_html=True
                    )
                ...
                if not df_result_recipe.empty:
                    ...
                    st.markdown(
                        '<div style="font-size:18px; font-family:Arial; color:#f0efa2; line-height:1.2; margin:2px 0;">📋 配方管理</div>',
                        unsafe_allow_html=True
                    )
                    st.dataframe(
    ...
    with tab4:
        ...
        # 初始化 session_state
        if "form_sample" not in st.session_state:
            st.session_state.form_sample = {"日期": "", "客戶名稱": "", "樣品編號": "", "樣品名稱": "", "樣品數量": ""}
        ...
        if "sample_limit" not in st.session_state:
            st.session_state.sample_limit = 5
        ...
        with col3:
            st.session_state.form_sample["樣品編號"] = st.text_input(
                "樣品編號",
        ...
        with col4:
            st.session_state.form_sample["樣品名稱"] = st.text_input(
                "樣品名稱",
                value=st.session_state.form_sample.get("樣品名稱", ""),
                key="sample_name"
            )
        with col5:
            st.session_state.form_sample["樣品數量"] = st.text_input(
                "樣品數量",
        ...
        if st.button("💾 儲存", key="save_sample"):
            new_data = st.session_state.form_sample.copy()
            if not new_data["樣品編號"].strip():
                st.warning("⚠️ 請輸入樣品編號！")
            else:
                if st.session_state.edit_sample_index is not None:
                    # 更新模式
                    idx = st.session_state.edit_sample_index
                    for col in df_sample.columns:
                        df_sample.at[idx, col] = new_data.get(col, "")
                    st.success("✅ 樣品記錄已更新！")
                else:
                    # 新增模式
                    df_sample = pd.concat([df_sample, pd.DataFrame([new_data])], ignore_index=True)
                    st.success("✅ 新增成功！")
...
# ===== 刪除確認 =====
    if st.session_state.show_delete_sample_confirm:
        target_row = df_sample.iloc[st.session_state.delete_sample_index]
        target_text = f'{target_row["樣品編號"]} {target_row["樣品名稱"]}'
        st.warning(f"⚠️ 確定要刪除 {target_text}？")
        c1, c2 = st.columns(2)
        if c1.button("刪除", key="confirm_delete_sample"):
            ...
            st.session_state.show_delete_sample_confirm = False
            st.rerun()
        if c2.button("取消", key="cancel_delete_sample"):
            st.session_state.show_delete_sample_confirm = False
            st.rerun()
