...
def generate_print_page_content_a5_special(order, recipe_row, additional_recipe_rows=None, show_additional_ids=True):
    ...
    pack_line = []
    for i in range(4):
        weight = 100 if i == 0 else float(order.get(f"包裝重量{i+1}", 0) or 0)
        count = float(order.get(f"包裝份數{i+1}", 0) or 0)
        if weight > 0 and count > 0:
            if i == 0:
                display_str = f"100K × {int(count)}"
            else:
                display_str = f"{int(weight)}kg × {int(count)}"
    ...
    for idx in range(8):
        ...
        if val in [None, "", 0, 0.0]:
            ...
            val_float = float(val)
            colorant_total += val_float  # 累加色粉重量
    ...
    total_line = f"{total_type_display}{' ' * 1}{remaining_str:>{number_col_width}}"
...
# ======== 客戶名單 =========
    ...
    if add_btn:
        if selected_label is None or selected_label == "請選擇" or selected_label == "（無符合配方）":
            ...
            if selected_row.get("狀態") == "停用":
                ...
                # 查找附加配方
                main_recipe_code = str(selected_row.get("配方編號", "")).strip()
                ...
                for i in range(1, 9):
                    ...
                    wt_val = selected_row.get(wt_key, 0)
                    if id_val or wt_val:
                ...
                # 填入包裝重量與份數 (測試值可先固定)
                test_weights = [0.5, 1, 2, 0]
                test_counts = [1, 1, 1, 0]
                for i in range(1, 5):
                    order[f"包裝重量{i}"] = float(order.get(f"包裝重量{i}", test_weights[i-1]))
                    order[f"包裝份數{i}"] = float(order.get(f"包裝份數{i}", test_counts[i-1]))
                ...
                for i in range(1, 9):
                    ...
                    if i <= len(all_colorants):
                        ...
                        order[id_key] = id_val or ""
                        try:
                            order[wt_key] = float(wt_val)
                        except:
                            order[wt_key] = 0
                    else:
                        order[id_key] = ""
    ...
    def generate_production_order_print(order, recipe_row, additional_recipe_rows=None, show_additional_ids=True):
        ...
        # 安全取得色粉類別
        category = str(order.get("色粉類別", "") or "").strip()
        
        # 其他欄位
        unit = recipe_row.get("計量單位", "kg")
        ratio = recipe_row.get("比例3", "")
        total_type = str(recipe_row.get("合計類別", "") or "").strip()
        if total_type == "原料":
            total_type = "料"
        ...
        packing_counts = []
        for i in range(1, 5):
            w = order.get(f"包裝重量{i}", 0)
            c = order.get(f"包裝份數{i}", 0)
            try:
                w = float(w)
            except:
                w = 0
        ...
        print(f"=== DEBUG ORDER ===")
        for i in range(1, 5):
            print(f"包裝重量{i}:", packing_weights[i-1])
            print(f"包裝份數{i}:", packing_counts[i-1])
        print("色粉類別:", category)
        print("計量單位:", unit)
        print("=== DEBUG RECIPE ===")
        colorant_ids = []
        colorant_weights = []
        for i in range(1, 9):
            cid = str(recipe_row.get(f"色粉編號{i}", "") or "").strip()
            cwt = recipe_row.get(f"色粉重量{i}", 0)
            try:
                cwt = float(cwt)
            except:
                cwt = 0
            colorant_ids.append(cid)
            colorant_weights.append(cwt)
            print(f"色粉編號{i}:", cid, "色粉重量{i}:", cwt)
        print("====================")
        ...
        color = order.get('顏色', '')
        pantone = order.get('Pantone 色號', '')
        info_line = f"<span style='font-size:20px;'>編號：<b>{recipe_id:<8}</b>顏色：{color:<4}"
        if ratio:
            info_line += f"   比例：{ratio} g/kg"
        if pantone:
            info_line += f"   Pantone：{pantone}"
        info_line += "</span>"
        ...
        for i in range(4):
            ...
            if w > 0 or c > 0:
                ...
                print(f"DEBUG 包裝{i+1}: {text}")  # debug
    
        if pack_line:
            lines.append("<b>" + " ".join(pack_line) + "</b>")
        ...
        for idx in range(8):
            ...
            for i in range(4):
                if category == "色母":
                    ...
                    val = c_weight * packing_weights[i] if packing_weights[i] > 0 else 0  # 非色母保持原本邏輯
...
# ---------- 選單區 ----------
def render_order_selectbox(df_order):
    options = df_order["生產單號"].astype(str).tolist() if not df_order.empty else []
    ...
    st.selectbox(
        "選擇生產單號",
        options=[""] + options,
        key="select_order_for_edit",
        on_change=update_selected_code
    )

# ---------- 三欄按鈕區 ----------
def render_production_order_buttons(df_order, df_recipe, ws_order=None, order_file=None):
    selected_code_edit = st.session_state.get("selected_code_edit", None)
    if not selected_code_edit or df_order.empty or df_recipe.empty:
        st.info("請先選擇生產單")
        return
