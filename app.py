...
if menu == "庫存區":
    ...
    if st.button("計算庫存", key="btn_calc_stock"):
        ...
        for pid in df_stock_copy["色粉編號"].unique():
            ...
            if s_dt is None:
                # 未選區間 → 從最早日期開始
                s_dt_eff = df_pid["日期"].min() if df_pid["日期"].notna().any() else pd.Timestamp.today()
            else:
                s_dt_eff = pd.to_datetime(s_dt)
