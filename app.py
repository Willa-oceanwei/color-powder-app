        ...
        if not df_oem.empty:
            ...
            if not oem_options:
                ...
                if selected_option:
                    ...
                    if st.session_state.get("oem_vendor") not in vendor_options:
                        st.session_state.oem_vendor = ""
...
# ======== 庫存區分頁 =========
    ...
    st.caption("分頁對照：📦 初始＝初始庫存設定｜📊 查詢＝庫存查詢｜📋 盤點＝庫存盤點分析｜🏆 排行＝色粉用量排行榜｜🧮 用量＝色粉用量查詢｜🧴 色母＝色母庫存查詢｜👤 客戶＝個別客戶庫存")
