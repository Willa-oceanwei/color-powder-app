...
# ======== 客戶名單 =========
    ...
    def generate_production_order_print(order, recipe_row, additional_recipe_rows=None, show_additional_ids=True):
        ...
        multipliers = packing_weights  # 對應每個包裝重量
        ...
        # 包裝列（色母固定 K）def generate_production_order_print(order, recipe_row, additional_recipe_rows=None, show_additional_ids=True):
    if recipe_row is None:
        recipe_row = {}

    category = (order.get("色粉類別") or "").strip()  # 色粉或色母
    ...
    colorant_ids = [recipe_row.get(f"色粉編號{i+1}", "") for i in range(8)]
    colorant_weights = [safe_float(recipe_row, f"色粉重量{i+1}") for i in range(8)]

    # 對應每個包裝重量的乘數
    multipliers = []
    for w in packing_weights:
        if category == "色母":
            multipliers.append(w * 100)  # 色母單位換算
        else:
    ...
    for i in range(4):
        ...
        if w > 0 or c > 0:
            if category == "色母":
                real_w = w * 100
                unit_str = f"{int(real_w)}K" if real_w == int(real_w) else f"{real_w:.0f}K"
            elif unit == "包":
                real_w = w * 25
                unit_str = f"{int(real_w)}K" if real_w == int(real_w) else f"{real_w:.0f}K"
            elif unit == "桶":
