...
with tab2:
	...
	if df.empty:
		...
		if "limit_per_page_tab2" not in st.session_state:
			st.session_state.limit_per_page_tab2 = 5
		...
		if st.session_state.last_limit_tab2 != limit:
			st.session_state.page_tab2 = 1
			st.session_state.last_limit_tab2 = limit
		...
		# ===== 表格顯示 =====
		show_cols = ["配方編號", "顏色", "客戶編號", "客戶名稱", "配方類別", "狀態", "Pantone色號"]
		...
		if not page_data.empty:
			...
			for _, row in page_data.iterrows():
				...
				with c1:
					st.markdown(
						f"**{row['配方編號']}**｜{row['客戶名稱']}｜{row['顏色']}"
					)

				with c2:
					if st.button(
						"👁️ 查看",
						key=f"view_recipe_{row['配方編號']}"
					):
						st.session_state.selected_recipe_id = row["配方編號"]
...
# ============================================================
	...
	with tab3:
	    ...
	    # 👉 接收 Tab 2 傳來的配方編號
	    selected_recipe_from_tab2 = st.session_state.get("selected_recipe_id")
	    ...
	    if not df_recipe.empty and "配方編號" in df_recipe.columns:
	        ...
	        # ⭐ 若 Tab2 有指定配方，找出對應 index
	        default_index = 0
	        if selected_recipe_from_tab2:
	            match = df_recipe[df_recipe["配方編號"] == selected_recipe_from_tab2]
	            if not match.empty:
	                default_index = options.index(match.index[0])
	        ...
	        selected_index = st.selectbox(
	            ...
	            f"{df_recipe.at[i, '配方編號']} | {df_recipe.at[i, '顏色']} | {df_recipe.at[i, '客戶名稱']}",
	            key="select_recipe_code_page_tab3"
	        )
	
	        selected_code = (
	            df_recipe.at[selected_index, "配方編號"]
	        ...
	        if selected_code:
	            ...
	            if not df_selected.empty:
	                ...
	                if st.session_state.get("show_edit_recipe_panel") and st.session_state.get("editing_recipe_index") is not None:
	                    ...
	                    with col3:
	                        options_cust = [""] + customer_options
	                        cust_id = fr.get("客戶編號", "").strip()
	                        cust_name = fr.get("客戶名稱", "").strip()
	                        current = f"{cust_id} - {cust_name}" if cust_id else ""
	                        index = options_cust.index(current) if current in options_cust else 0
	                        selected = st.selectbox("客戶編號", options_cust, index=index, key="edit_recipe_selected_customer_tab3")
	                        if " - " in selected:
	                            c_no, c_name = selected.split(" - ", 1)
	                            fr["客戶編號"] = c_no
	                            fr["客戶名稱"] = c_name
	                    ...
	                    with col11:
	                        options_unit2 = ["g", "kg"]
	                        current = fr.get("淨重單位", options_unit2[0])
	                        if current not in options_unit2:
	                            current = options_unit2[0]
	                        fr["淨重單位"] = st.selectbox("單位", options_unit2, index=options_unit2.index(current), key="edit_recipe_net_unit_tab3")
