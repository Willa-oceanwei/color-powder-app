...
if menu == "代工管理":
    ...
    # 若舊版工作表沒有「已交貨」欄位，自動補上（只做一次）
    try:
        oem_headers = ws_oem.row_values(1)
        if "已交貨" not in oem_headers:
            ws_oem.update_cell(1, len(oem_headers) + 1, "已交貨")
    except:
        pass
    ...
    with tab5:
        ...
        if not search_client and not search_recipe:
            ...
            if df_progress.empty:
                ...
                st.markdown("---")
                st.markdown("#### ✅ 已結案品項交貨註記")
                df_closed = df_display[df_display["狀態"] == "✅ 已結案"].copy()
                if df_closed.empty:
                    st.info("目前查詢結果沒有已結案品項")
                else:
                    ...
                    edited_closed = st.data_editor(
                        df_closed[editor_cols],
                        use_container_width=True,
                        hide_index=True,
                        column_config={
                            "已交貨": st.column_config.CheckboxColumn("已交貨", help="僅做註記，不影響其他流程")
                        },
                        disabled=[c for c in editor_cols if c != "已交貨"],
                        key="oem_closed_delivery_editor"
                    )

                    if st.button("💾 儲存已交貨註記", key="save_closed_delivery_flag"):
                        all_values = get_cached_sheet_values("代工管理")
