...
# ======== 客戶名單 =========
    ...
    with col1:
        search_keyword = st.text_input("全欄模糊搜尋（配方編號 / Pantone / 客戶）", key="search_keyword")
    with col2:
    ...
    with st.expander("進階搜尋"):
        ...
        advanced_customer = st.text_input("客戶編號或名稱", key="advanced_customer")

    # ------ 2. filter邏輯（合併條件）------
    df_filtered = df.copy()

    # 進階搜尋條件（任何一個有值都用 AND 疊加！）
    if st.session_state.get("advanced_recipe",'').strip():
        val = st.session_state["advanced_recipe"].strip()
        df_filtered = df_filtered[df_filtered["配方編號"].str.contains(val, case=False, na=False)]
    if st.session_state.get("advanced_pantone",'').strip():
        val = st.session_state["advanced_pantone"].strip()
        df_filtered = df_filtered[df_filtered["Pantone色號"].str.contains(val, case=False, na=False)]
    if st.session_state.get("advanced_customer",'').strip():
        val = st.session_state["advanced_customer"].strip()
        df_filtered = df_filtered[
            df_filtered["客戶編號"].str.contains(val, case=False, na=False) |
            df_filtered["客戶名稱"].str.contains(val, case=False, na=False)
        ]
    # 只有全部進階條件都空時，才啟用全欄模糊
    if not (
        st.session_state.get("advanced_recipe") or
        st.session_state.get("advanced_pantone") or
        st.session_state.get("advanced_customer")
    ):
